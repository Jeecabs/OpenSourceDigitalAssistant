/**
 * Note: This module involves connect/scanner for the Python modules and sending them back to the client.
 * Since this fork will be a lighter Javascript only approach this section will need to be reworked to fit in that paradigm.
 * 
 * 
 * 
 */
...
downloadRouter.get("/", function (req, res, _next) {
  log.title("GET /downloads");

  const clean = (dir: string, files: string[]) => {
  ...
  if (req.query.package && req.query.module) {
    ...
    log.info(
      `Checking existence of the ${string.ucfirst(
        req.query.module.toString()
      )} module...`
    );
    if (fs.existsSync(module)) {
      log.success(
        `${string.ucfirst(req.query.module.toString())} module exists`
      );
