...
//import DeepSpeech from "deepspeech-gpu";
...
let model: undefined | Model;

/**
 * Model and language model paths
 */
const conf = {
  model: "bin/deepspeech/deepspeech-0.9.3-models.pbmm",
...
function init(args: {
  model: fs.PathLike;
  alphabet: fs.PathLike;
  lm: fs.PathLike;
  trie: fs.PathLike;
}) {
  if (process.env.LEON_LANG !== "en-US") {
    warningLog(
      'The DeepSpeech parser only accepts the "en-US" language for the moment'
    );
  }
  ...
  if (!fs.existsSync(args.alphabet)) {
    log.error(
      `Cannot find ${args.alphabet}. You can setup the offline STT by running: "npm run setup:offline-stt"`
    );
  ...
  if (process.env.LEON_NODE_ENV !== "testing") {
    log.info(`MODLE PATH ${args.model}`);
    model = new Model(
      args.model.toString()
      //N_FEATURES,
      //N_CONTEXT,
  ...
  if (args.lm && args.trie) {
    log.info(
      `Loading language model from files ${args.lm} and ${args.trie}...`
    );

    if (!fs.existsSync(args.lm)) {
      log.error(
        `Cannot find ${args.lm}. You can setup the offline STT by running: "npm run setup:offline-stt"`
      );
    ...
    if (!fs.existsSync(args.trie)) {
      log.error(
        `Cannot find ${args.trie}. You can setup the offline STT by running: "npm run setup:offline-stt"`
      );
 ...
 */
function parse(buffer: Buffer, cb: Function) {
  ...
  if (process.env.LEON_NODE_ENV !== "testing") {
    if (model != undefined) {
      const string = model.stt(buffer.slice(0, buffer.length / 2));
