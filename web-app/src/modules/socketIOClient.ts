import { Socket } from "socket.io-client";
import { io } from "socket.io-client";
import Recorder from "./Recorder";

export default class Client {
  client: string;
  serverUrl: string;
  history: string | null;
  parsedHistory: never[];
  info: any;
  _recorder: Recorder | null;
  socket: Socket;
  chatbot: any;
  handleResponse: Function;
  constructor(
    client: string,
    serverUrl: string,
    res: any,
    setResponseMessage: Function
  ) {
    this.client = client;
    this.serverUrl = serverUrl;
    this.socket = io(serverUrl);
    this.history = localStorage.getItem("history");
    this.parsedHistory = [];
    this.info = res;
    this._recorder = null;
    this.handleResponse = setResponseMessage;
  }

  setRecorder = (recorder: any) => {
    this._recorder = recorder;
  };
  init() {
    this.socket.on("FromAPI", (data: any) => {});
    // this.chatbot.init();

    this.socket.on("connect", () => {
      this.socket.emit("init", "webapp");
    });

    this.socket.on("answer", (data: any) => {
      console.log("ANSWER");
      console.log(data);
      this.handleResponse(data);
      //this.chatbot.receivedFrom("leon", data);
    });

    this.socket.on("is-typing", (data: any) => {
      //this.chatbot.isTyping("leon", data);
    });

    this.socket.on("recognized", (data: any, cb: Function) => {
      // TODO FIGURE WHAT THIS IS MEANT TO DO
      //this._input.value = data;
      console.log("NOT IMPLEMENTED");
      this.send("query", "NOT IMPLEMENTED");

      cb("string-received");
    });

    this.socket.on(
      "audio-forwarded",
      (
        data: { buffer: ArrayBuffer; is_final_answer: any; duration: number },
        cb: (arg0: string) => void
      ) => {
        const ctx = new AudioContext();
        const source = ctx.createBufferSource();

        ctx.decodeAudioData(data.buffer, (buffer) => {
          source.buffer = buffer;

          source.connect(ctx.destination);
          source.start(0);

          /**
           * When the after speech option is enabled and
           * the answer is a final one
           */
          if (this.info.after_speech && data.is_final_answer) {
            // Enable recording after the speech + 500ms
            setTimeout(() => {
              if (this._recorder == null) return;
              this._recorder.start();
              this._recorder.enabled = true;

              // Check every second if the recorder is enabled to stop it
              const id = setInterval(() => {
                if (this._recorder == null) return;
                if (this._recorder.enabled) {
                  if (this._recorder.countSilenceAfterTalk <= 8) {
                    // Stop recording if there was no noise for 8 seconds
                    if (this._recorder.countSilenceAfterTalk === 8) {
                      this._recorder.stop();
                      this._recorder.enabled = false;
                      this._recorder.countSilenceAfterTalk = 0;
                      clearInterval(id);
                    } else if (!this._recorder.noiseDetected) {
                      this._recorder.countSilenceAfterTalk += 1;
                    } else {
                      clearInterval(id);
                    }
                  }
                }
              }, 1000);
            }, data.duration + 500);
          }
        });

        cb("audio-received");
      }
    );
    this.socket.on("download", (data: any) => {
      console.log("BOSS CHECK THIS");
      console.log(
        `${this.serverUrl}/v1/downloads?package=${data.package}&module=${data.module}`
      );

      //window.location = `${this.serverUrl}/v1/downloads?package=${data.package}&module=${data.module}`;
    });
  }
  send = (keyword: string, message: string) => {
    this.socket.emit(keyword, {
      client: this.client,
      value: message.trim(),
    });
    //this.chatbot.sendTo("leon", this._input.value);

    this.save();

    return true;
  };

  save = () => {
    // let val = this._input.value;

    if (localStorage.getItem("history") === null) {
      localStorage.setItem("history", JSON.stringify([]));
      // this.parsedHistory = JSON.parse(localStorage.getItem("history"));
    } else if (this.parsedHistory.length >= 32) {
      this.parsedHistory.shift();
    }

    // if (val[0] === " ") {
    //  val = val.substr(1, val.length - 1);
    //}

    // if (this.parsedHistory[this.parsedHistory.length - 1] !== val) {
    //   this.parsedHistory.push(val);
    //   localStorage.setItem("history", JSON.stringify(this.parsedHistory));
    // }

    //this._input.value = "";
  };
}
